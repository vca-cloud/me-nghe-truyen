...
export function CategoryManager() {
  ...
  const save = async () => {
    ...
    const result = editing
      ? await adminWrite({ table: "categories", op: "update", values: { name, slug }, match: { id: editing.id } })
      : await adminWrite({ table: "categories", op: "insert", values: { name, slug } })
    if (result.error) {
      toast.error(`Lỗi lưu thể loại: ${result.error.message}`)
      return
    }
    toast.success("Đã lưu thể loại"); setOpen(false); await load()
  }
  const toggle = async (item: Category) => { const { error } = await adminWrite({ table: "categories", op: "update", values: { is_visible: !item.is_visible }, match: { id: item.id } }); if (error) toast.error(error.message); else await load() }
  const remove = async (item: Category) => { if (!confirm(`Xóa thể loại ${item.name}?`)) return; const { error } = await adminWrite({ table: "categories", op: "delete", match: { id: item.id } }); if (error) toast.error(`Lỗi xóa: ${error.message}`); else { toast.success("Đã xóa thể loại"); await load() } }
  return <>
