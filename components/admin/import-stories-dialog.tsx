...
async function findExistingStory(title: string, slug: string) {
  const bySlug = await supabase.from("stories").select("id, slug, title").eq("slug", slug).maybeSingle()
  if (bySlug.data?.id) return bySlug.data

  const byTitle = await supabase.from("stories").select("id, slug, title").eq("title", title).maybeSingle()
  if (byTitle.data?.id) return byTitle.data
...
export function ImportStoriesDialog({ onSuccess }: Props) {
  ...
  const processImport = async (stories: ParsedStory[], parseErrors: ParseError[]) => {
    ...
    for (let index = 0; index < stories.length; index++) {
      ...
      try {
        const slug = slugify(story.title) || `truyen-${Date.now()}-${index}`
        const payload = {
          title: story.title,
          slug,
          author: story.author,
        ...
        const existing = await findExistingStory(story.title, slug)

        let storyId: number
        if (existing?.id) {
          const { error: updateError } = await supabase.from("stories").update(payload).eq("id", existing.id)
          if (updateError) {
            skipped++
            errors.push({ line: index + 1, field: "title", message: `Update lỗi: ${updateError.message}` })
            continue
          }
          storyId = existing.id
          await supabase.from("episodes").delete().eq("story_id", storyId)
        } else {
          let { data, error } = await supabase.from("stories").upsert(payload, { onConflict: "slug" }).select("id")
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Import từ CSV hoặc JSON. Mỗi dòng CSV là một tập. Truyện trùng title/slug sẽ được cập nhật, không tạo bản mới.</DialogDescription>
        </DialogHeader>
        <div className="space-y-5 py-4">
          ...
          {results && (
            <div className="rounded-md border border-green-200 bg-green-50 p-4 text-sm">
              ...
              <p>Thành công: {results.success}</p>
